  ...
  if (loading) {
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '80vh' }}>
        <CircularProgress size={60} thickness={4} sx={{ color: '#128c7e', mb: 3 }} />
  ...
  return (
    <Fade in={!loading} timeout={800}>
      <Box sx={{ 
        ...
        backgroundImage: waBackground,
        borderRadius: 3,
        p: { xs: 2, sm: 4 },
        minHeight: 'calc(100vh - 120px)',
        boxShadow: 'inset 0 0 20px rgba(0,0,0,0.02)'
      }}>
        <Typography variant="h4" gutterBottom sx={{ fontWeight: 'bold', color: '#075e54', mb: 4 }}>
          Analytics Dashboard
        </Typography>
        ...
        <Grid container spacing={3} mb={4}>
          {cards.map((card) => (
            <Grid size={{ xs: 12, sm: 6, md: 3 }} key={card.name}>
              <Card sx={{ borderRadius: 3, boxShadow: '0 4px 12px rgba(0,0,0,0.05)', transition: 'transform 0.2s', '&:hover': { transform: 'translateY(-4px)' } }}>
                <CardContent sx={{ display: 'flex', alignItems: 'center', p: 3 }}>
                  <Box sx={{ backgroundColor: card.bg, color: card.color, p: 1.5, borderRadius: 2, display: 'flex', mr: 2 }}>
                    {card.icon}
                  </Box>
                  <Box>
        ...
        <Card sx={{ borderRadius: 3, boxShadow: '0 4px 12px rgba(0,0,0,0.05)', p: 2 }}>
          <CardContent>
            <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bold', color: '#075e54', mb: 3 }}>
            ...
            <Box sx={{ height: 350, width: '100%' }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={stats.revenueData}>
                  ...
                  />
                  <Area type="monotone" dataKey="revenue" stroke="#128c7e" strokeWidth={3} fillOpacity={1} fill="url(#colorRevenue)" />
                </AreaChart>
              </ResponsiveContainer>
            </Box>
          </CardContent>
